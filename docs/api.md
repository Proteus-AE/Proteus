...
# operator groups of one transformer block: w.block
#   Operator(name, kind, bytes, flops, n_vectors, k_dim, n_out, reuse,
#            tokens) with .intensity = Eq. (3) for shared operands and the
#   query-group size for attention
# per-layer expansion for reporting: w.operators
# routing histogram of this iteration (sampled mode): w.routing_hist
```
...
r = sys_.simulate(w, devices=None, dp=1, frozen_plan=None)   # -> Result
# Proteus-Static: freeze the schedule at a deployment-time reference
plan = sys_.deployment_plan(reference_workload)
r_static = sys_.simulate(w, frozen_plan=plan)
# topology helpers
n_tp, groups = sys_.topology(devices)
layers, imbalance = sys_.layer_partition(n_layers, groups)
# capacity interface, shared with every baseline
sys_.total_capacity(devices); sys_.kv_capacity(model_cfg, devices)
...
```python
binary(name="pimcore")        # path of a built binary; raises with the
                              # build command if `make core` has not been run
available(name="pimcore")     # same check, without raising
