#!/usr/bin/env python3
"""Overall results (Fig. "OverallResults"): throughput and energy efficiency
...
def main():
    ...
    for m in MODELS:
        for b in BATCHES:
            ...
            thr_abs.append([b] + [round(thr[s]) for s in SYSTEMS])
            eff_abs.append([b] + [round(eff[s], 2) for s in SYSTEMS])
            thr_norm.append([b] + [round(thr[s] / base_t, 4) for s in SYSTEMS])
            eff_norm.append([b] + [round(eff[s] / base_e, 4) for s in SYSTEMS])
            for s in speedup_vs:
                if thr[s] > 0:
    ...
    hdr = ["batch"] + [SYSTEM_LABELS[s] for s in SYSTEMS]
